...
We calculated the proportion of target fixations for each participant, in each condition and submitted these proportions to a zero-and-one inflated beta regression model. 
The outcome was modeled as a function of group (HL, L2), stress (paroxytone, oxytone), use, and proficiency, along with all higher order interactions. 
...
@fig-eda-prof-use-marginal-mean-estimates-report
...
bi_zoid_tib %>% 
  mutate(Effect = case_when(
    Term != "Intercept" & Effect == "Population" ~ " ", 
    Term != "sd(Intercept)" & Effect == "Grouping" ~ " ", 
    TRUE ~ Effect
  )) %>% 
  knitr::kable(format = "pandoc", 
    align = c("l", "l", "r", "r", "r", "r", "r"))
